<div>
<x-admin.accounts-shell title="Akun Referral">
    ...
    <div class="mb-5 flex flex-wrap items-center gap-sm">
        ...
        <span class="text-[13px] leading-[1.4] text-muted">
            {{ $this->accounts->total() }} akun · registrasi mandiri
        </span>
    </div>
            ...
            <tr wire:key="referral-{{ $account->id }}">
                ...
                <x-ui.td align="right">
                    <button type="button" wire:click="edit({{ $account->id }})"
                            class="text-[13px] font-medium text-link">Ubah profil</button>
                </x-ui.td>
            </tr>
        @empty
            <tr>
                <td colspan="6" class="px-5 py-lg text-center text-body-md text-muted">
                    @if ($search !== '')
                        Tidak ada Referral yang cocok dengan &ldquo;{{ $search }}&rdquo;.
                    @else
    ...
    <p class="mt-sm text-helper text-muted">
        Halaman ini hanya memuat profil. Aplikasi milik Referral tidak dapat dilihat Admin.
    </p>

    {{-- Edit panel --}}
    @if ($editingId)
        <form wire:submit="save" class="mt-lg">
            <x-ui.card title="Ubah Profil Referral">
