...
final readonly class ProductConfig
{
     ...
     * @param  array<int, float|null>  $flatRatesAddb  as printed on the rate card, per tenor
     * @param  array<int, float|null>  $flatRatesAddm
     */
    public function __construct(
        public string $name,
        public array $effectiveRates,
        public float $adminMax,
        public array $flatRatesAddb = [],
        public array $flatRatesAddm = [],
    ...
    /**
     * The flat rate as MTF printed it, or null when the card has not been
     * entered for this product. Null means the caller converts from the
     * effective rate instead and accepts the rounding drift that carries.
     */
    public function flatRateFor(int $tenorMonths, InstalmentType $instalmentType): ?float
    {
        $rates = $instalmentType === InstalmentType::ADDM ? $this->flatRatesAddm : $this->flatRatesAddb;

        return $rates[$tenorMonths] ?? null;
    }
    ...
    public function withUpping(float $upRate, float $upAdmin, float $upProvision): self
    {
        return new self(
            ...
            flatRatesAddb: $this->flatRatesAddb,
            flatRatesAddm: $this->flatRatesAddm,
